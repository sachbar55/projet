...
class Product(db.Model):
    ...
    def image_url(self):
        if self.images:
            return url_for("static", filename=f"uploads/{self.images[0].filename}")
    ...
    if request.method == "POST":
        ...
        for file in files:
            if file and file.filename:
                ...
                if saved:
                    img = ProductImage(product_id=product.id, filename=saved, position=position)
                    db.session.add(img)
                    position += 1
    ...
    if request.method == "POST":
        ...
        for file in files:
            if file and file.filename:
                ...
                if saved:
                    max_pos += 1
                    img = ProductImage(product_id=product.id, filename=saved, position=max_pos)
                    db.session.add(img)
...
@login_required
def admin_product_delete_image(image_id):
